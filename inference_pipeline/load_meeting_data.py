...
def load_data(meetings_dir: str, session_query: Optional[str] = None,
    ...
    for meeting_subdir in sorted_dirs:
        ...
        # expect a transcription file exists
        ...
        devices_df['session_id'] = meeting_subdir.name + '_' + devices_df['device_name']
    ...
    all_gt_utt_df = pd.concat(gt_utt_dfs, ignore_index=True)
    all_session_df = pd.concat(session_dfs, ignore_index=True)
    all_metadata_df = pd.concat(metadata_dfs, ignore_index=True)

    # MtgType column is useful for querying, but it is on the metadata df. merge it into session df.
    merged_df = all_session_df.merge(all_metadata_df[['meeting_id', 'MtgType']], on='meeting_id', how='inner')
